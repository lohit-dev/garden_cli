...
impl OrderService {
    pub fn new() -> Self {
        ...
        Self {
            client: Client::new(),
            api_url: String::from("https://relayerv2.garden.finance"),
            api_key: String::from(
                "AAAAAGnnHDw6OuFGHwJsgTQFutrJ4__itFiulz-_iRsD-c-OBaOhqZ4KWhB98QhtVYybp2UFWcv8O2LIoB7EmL77dkeQZ_4isbYc",
            ),
        }
    ...
    pub async fn create_order(
        ...
        let mut order = Order {
            ...
            initiator_source_address:
                "b15dcbdaa857b59c44107f09021c85cb6e9b4d1bee9b90a922f1f14a2ae8e44a".to_string(),
            initiator_destination_address:
                "0x02Ff86d284eabA0f21B2B884cfE15128f8251fb2737800aE9b973832F67B69B9".to_string(),
            source_amount,
        ...
        let res = self
            ...
            .header("Authorization", "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoiMHgxNTgwNjVjNTBjN0U0NmJBYzIwNjFFMTU3RkMxMTJEOTU0QTZkMzkxIiwiZXhwIjoxNzQ2MDEyNTcwfQ.E3FLFxhO7Fq_3dUvCIQ8THHKZ--cjCDwlP8pPJBddmE")
            .json(&order)
            .send()
            .await?;
    ...
    pub async fn get_quote(
        ...
        let url = format!(
            ...
            "https://pricev2.garden.finance?order_pair={}&amount={}&exact_out={}",
    ...
    pub async fn fetch_attested_quote(
        ...
        let response = self
            ...
            .post("https://pricev2.garden.finance/attested")
            .header("accept", "application/json")
            .header("Content-Type", "application/json")
            .header("api-key", &self.api_key)
            .json(&payload)
        ...
        loop {
            match operation().await {
                ...
                Err(e) => {
                    ...
                    retries += 1;
                    warn!(
                        "⚠️ Attempt {} failed for order {}: {}. Retrying with linear backoff in {} seconds...",
            ...
            self.retry_with_backoff(
                || async {
                    ...
                    let url = if is_starknet_source {
                        ...
                        "https://starknet-relay.garden.finance/initiate".to_string()
                    } else {
    ...
    async fn get_order_details(&self, order_id: &str) -> Result<OrderDetails> {
        ...
        let url = format!(
            ...
            "https://testnet.api.garden.finance/orders/id/matched/{}",
            order_id
        );
    ...
    pub async fn redeem_order(&self, order_id: &str, secret: &str) -> Result<String> {
        ...
        let url = if is_starknet_destination {
            ...
            "https://starknet-relay.garden.finance/redeem"
        } else {
            // "https://evm-relay-stage.hashira.io/redeem"
            "https://relayer.garden.finance/redeem"
        };
    ...
    pub async fn retry_redeem_order(
        ...
        for attempt in 1..=max_attempts {
            ...
            // Wait before the next attempt (Additional backoff)
            // Linear: 2, 4, 6, 8, 10 seconds...
            let wait_time = std::time::Duration::from_secs(2 * attempt as u64);
            info!(
                "⏱️ Waiting {:?} before next linear backoff attempt",
                wait_time
            );
            tokio::time::sleep(wait_time).await;
        }

        // This should never be reached due to the returns in the loop, but just in case, I can't trust myself
        Err(eyre::eyre!(
            "Failed to redeem order after {} attempts",
            max_attempts
        ))
